...
from RLQO.PPO_v3.config.query_action_mapping_v3 import PHASE1_ACTIONS, get_query_type


class QueryPlanDBEnvPPOv3(QueryPlanDBEnvV3):
    """
    PPO v3 RealDB 환경
    ...
    def __init__(self, 
        ...
        if self.verbose:
            print(f"[INFO] PPO v3 RealDB Environment initialized")
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.verbose:
            print(f"[INFO] Query type: {self.current_query_type_ppo}")
    ...
    def get_action_mask(self) -> np.ndarray:
        ...
        current_query_idx = self.current_query_ix % len(self.query_list)
        ...
        allowed_actions = self.phase1_actions.get(self.current_query_type_ppo, [])
    ...
    def step(self, action_id):
        ...
        if not info.get('invalid_action', False):
            ...
            if self.verbose:
                print(f"[REWARD] Normalized: {reward:.3f} (Type: {self.current_query_type_ppo}, Action: {action_name})")
        else:
...
if __name__ == '__main__':
    ...
    env.close()
    print("\n[SUCCESS] PPO v3 RealDB 환경 테스트 완료!")
