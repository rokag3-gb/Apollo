# -*- coding: utf-8 -*-
"""
Ensemble v1 (4-Model) 결과 분석 스크립트
...
for q_idx_str, query_data in results['query_results'].items():
    ...
    # 모델별 예측 분석 (첫 번째 episode 기준)
    first_detail = query_details[0]
    model_predictions = first_detail.get('predictions', {})
...
# 전체 출력
print("=" * 150)
print("Ensemble v1 (4-Model) 쿼리별 성능 분석")
print("=" * 150)
...
# Action diversity 계산
action_diversity = len(action_counts)
print(f"\nAction Diversity: {action_diversity} different actions used")
print("=" * 150)
print()
...
# 3-모델 vs 4-모델 비교
print("=" * 150)
print("3-Model vs 4-Model Comparison")
print("=" * 150)
...
try:
    ...
    print(f"{'Metric':<20} {'3-Model':<15} {'4-Model (DQN v4 Added)':<25} {'Improvement':<15}")
    print("-" * 80)
    ...
    metrics = [
        ...
        ('Win Rate', 'win_rate', '%'),
        ('Safe Rate', 'safe_rate', '%'),
        ('Model Agreement', 'mean_agreement', '%'),
    ]
    ...
    for metric_name, metric_key, unit in metrics:
        ...
        if unit == '%':
            ...
            diff = (val_4m - val_3m) * 100
            diff_display = f"{diff:+.1f}%p"
        else:
            val_3m_display = f"{val_3m:.3f}{unit}"
            val_4m_display = f"{val_4m:.3f}{unit}"
            diff = val_4m - val_3m
            diff_display = f"{diff:+.3f}{unit}"
        
        print(f"{metric_name:<20} {val_3m_display:<15} {val_4m_display:<25} {diff_display:<15}")
    
    print("=" * 150)
except:
    print("3-Model results not found for comparison")
    print("=" * 150)

# CSV로도 저장
csv_file = os.path.join(os.path.dirname(__file__), 'results', 'ensemble_4models_summary.csv')
