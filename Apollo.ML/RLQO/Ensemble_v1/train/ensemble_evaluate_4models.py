# -*- coding: utf-8 -*-
"""
Ensemble v1: 4-Model Evaluation (DQN v4 추가)

DQN v4, PPO v3, DDPG v1, SAC v1 4개 모델로 30개 쿼리 평가
"""
...
# Ensemble_v1을 마지막에 추가 (config 이름 충돌 방지)
sys.path.insert(0, project_root)
sys.path.insert(0, apollo_ml_dir)
sys.path.insert(0, rlqo_dir)
sys.path.append(ensemble_dir)  # append로 변경 (우선순위 낮춤)
...
class FourModelEnsemble:
    """
    DQN v4, PPO v3, DDPG v1, SAC v1 4개 모델로 구성된 Ensemble
    각 모델이 독립적인 환경을 사용
    """
    ...
    def load_models_and_envs(self, queries: List[str], max_steps: int = 1):
        """
        4개 모델과 각 모델의 환경을 로드합니다.
        """
        if self.verbose:
            print("=" * 80)
            print("Loading 4 Models and Environments")
            print("=" * 80)
        ...
        try:
            ...
            if os.path.exists(MODEL_PATHS['dqn_v4']):
                self.models['dqn_v4'] = DQN.load(MODEL_PATHS['dqn_v4'])
                self.envs['dqn_v4'] = QueryPlanDBEnvV4(
                    query_list=queries,
                    max_steps=max_steps,
                    verbose=False
                )
                ...
                if self.verbose:
                    print(f"[OK] DQN v4 loaded with QueryPlanDBEnvV4")
            else:
                if self.verbose:
                    print(f"[X] DQN v4 not found at {MODEL_PATHS['dqn_v4']}")
    ...
    def predict(self, query_idx: int, episode: int):
        ...
        Returns:
            ...
            baseline_time: 첫 번째 환경의 baseline 시간
        """
        predictions = {}
        confidences = {}
        observations = {}
        ...
        for model_name in self.loaded_models:
            try:
                ...
                # Get baseline from first model's info
                if baseline_time is None and 'metrics' in info:
...
def evaluate_4model_ensemble(
    ...
    """
    4-Model Ensemble 평가 (DQN v4, PPO v3, DDPG v1, SAC v1)
    """
    
    if verbose:
        print("=" * 80)
        print(f"4-Model Ensemble Evaluation: {voting_strategy.upper()}")
        print("=" * 80)
        print(f"Models: DQN v4 (NEW), PPO v3, DDPG v1, SAC v1")
        print(f"Queries: {n_queries} (Q0-Q{n_queries-1})")
        print(f"Episodes: {n_episodes}")
        print("=" * 80 + "\n")
    
    # Create ensemble
    ensemble = FourModelEnsemble(voting_strategy=voting_strategy, verbose=verbose)
        ...
        for episode in range(n_episodes):
            try:
                ...
                if baseline_time is None:
                    baseline_time = episode_baseline
...
def main():
    ...
    print("=" * 80)
    print("4-Model Ensemble: 30 Query Evaluation (DQN v4 Added!)")
    print("=" * 80)
    print("Models: DQN v4 (NEW), PPO v3, DDPG v1, SAC v1")
    print("Queries: 30 (constants2.py)")
    print("=" * 80 + "\n")
    
    # Full evaluation (30 queries × 10 episodes = 300 evaluations)
    results = evaluate_4model_ensemble(
    ...
    # Save results
    output_file = os.path.join(os.path.dirname(__file__), '..', 'results', 'ensemble_4models_30queries.json')
