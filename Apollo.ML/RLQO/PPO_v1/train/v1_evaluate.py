    ...
    for q_idx, query in enumerate(queries):
        ...
        for run in range(num_runs):
            try:
                ...
                # 모델이 액션을 선택 (MaskablePPO는 action_masks를 사용)
                action, _ = model.predict(obs, deterministic=True, action_masks=action_mask.astype(bool))
                ...
                print(f"    Run {run+1}: {agent_time:.2f} ms (action: {action}, reward: {reward:.3f})")
...
def compare_models(models_to_evaluate, mode='full'):
    ...
    if all_summaries:
        ...
        results_file = f"Apollo.ML/artifacts/RLQO/evaluation_results_ppo_v1_{timestamp}.csv"
        summary_file = f"Apollo.ML/artifacts/RLQO/evaluation_summary_ppo_v1_{timestamp}.csv"
...
def quick_test_ppo():
    ...
    if len(valid_actions) > 0:
        ...
        print(f"\nValid action 테스트: {base_env.actions[action]['name']}")
        obs, reward, terminated, truncated, info = env.step(action)
        print(f"Reward: {reward:.4f}")
        print(f"Metrics: {info.get('metrics', {})}")
    
    env.close()
    print("\n[SUCCESS] PPO v1 환경 테스트 완료!")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='PPO v1 모델 평가')
    parser.add_argument('--model', type=str, default='Apollo.ML/artifacts/RLQO/models/ppo_v1_sim.zip',
                       help='평가할 모델 경로')
    ...
    if not existing_models:
        print("[ERROR] No PPO v1 models found for evaluation!")
        print("Available models:")
        model_dir = 'Apollo.ML/artifacts/RLQO/models/'
        if os.path.exists(model_dir):
            for f in os.listdir(model_dir):
                if f.endswith('.zip') and 'ppo' in f.lower():
                    print(f"  - {f}")
        return
