...
def analyze_dataset():
    ...
    print(f"원본 데이터 크기: {df_plans.shape}")
    print(f"피처 데이터 크기: {df_features.shape}")
    ...
    axes[0,0].set_title('Target Distribution (last_ms)')
    ...
    axes[0,1].set_title('Target Distribution (log scale)')
    ...
    plt.tight_layout()
    plt.savefig('artifacts/target_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    correlations = df_features[numeric_cols + ['last_ms']].corr()['last_ms'].sort_values(key=abs, ascending=False)
    print("상위 10개 상관관계:")
    print(correlations.head(11)[1:])  # last_ms 자기 자신 제외
    
    # 5. 상관관계 히트맵
    plt.figure(figsize=(12, 10))
    corr_matrix = df_features[numeric_cols + ['last_ms']].corr()
    ...
    # 피처별 분산 분석
    feature_vars = df_features[numeric_features].var().sort_values(ascending=False)
    print("\n피처별 분산 (상위 10개):")
    ...
    missing_percent = (missing_values / len(df_features)) * 100
    ...
    # 0값이 많은 피처들
    zero_counts = (df_features[numeric_features] == 0).sum()
    high_zero_features = zero_counts[zero_counts > len(df_features) * 0.8]
    print(f"80% 이상이 0인 피처들: {len(high_zero_features)}개")
    if len(high_zero_features) > 0:
        print(high_zero_features)
    
    # 상수 피처들
    constant_features = df_features[numeric_features].nunique() == 1
    print(f"상수 피처들: {constant_features.sum()}개")
    if constant_features.any():
        print(constant_features[constant_features].index.tolist())
    
    return df_plans, df_features, correlations

if __name__ == "__main__":
    df_plans, df_features, correlations = analyze_dataset()